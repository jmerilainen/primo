...
export const WeatherItem = (props) => {
    ...
    return (
        <div className={`p-6 flex flex-col items-center transition ease-in-out duration-700 transform ${! visible ? 'translate-y-8 opacity-0' : ''} hover:-translate-y-2 group`}>
            ...
            <FeatherIcon icon={icon} size="24" className="transition ease-in-out duration-700 group-hover:text-t-primary" />
            <div className="text-xs py-4 text-t-primary transition ease-in-out duration-700 transform opacity-50 scale-75 -translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 group-hover:scale-100">
